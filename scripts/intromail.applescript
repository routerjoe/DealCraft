...
on run
	...
	set recSubjIdx to my headerIndex("recommended_subject", headers)
	...
	tell application "Microsoft Outlook"
		repeat with i from 2 to count of csvLines
			...
			if thisLine is not "" then
				...
				-- Prefer per-row recommended_subject if present
				if recSubjIdx ≤ (count of fields) then
					set recSubj to item recSubjIdx of fields
					if recSubj is not "" then set subj1 to recSubj
				end if
