...
class IntroMailScreen(Screen):
    ...
    async def refresh_campaigns(self):
        ...
        try:
            ...
            if not campaigns:
                self._write_log("[dim]No analyzed campaigns found yet. Analyze a CSV to get started.[/dim]")
    ...
    async def action_select_csv(self):
        ...
        if selected:
            ...
            self._write_log("[yellow]⚠️  Please select a campaign from the table or enter a path[/yellow]")
    ...
    async def _run_analyze(self, csv_path: str):
        ...
        self._write_log(f"[b][cyan]📊 Analyzing Campaign CSV[/cyan][/b]")
            ...
            if result.get("success"):
                ...
                if "content" in data and len(data["content"]) > 0:
                    ...
                    if response_data.get("ok"):
                        ...
                        self._write_log(f"[b]Results:[/b]")
                        self._write_log(f"  • Total contacts: {totals.get('rows', 0)}")
                        self._write_log(f"  • High priority: [green]{totals.get('high', 0)}[/green]")
                        self._write_log(f"  • Medium priority: [yellow]{totals.get('medium', 0)}[/yellow]")
                        ...
                        self._write_log(f"[yellow]⚠️  {response_data.get('message', 'Analysis completed')}[/yellow]")
    ...
    async def _run_generate(self, csv_path: str, subject_template: str = None):
        ...
        self._write_log(f"\n[b][yellow]{'─' * 80}[/yellow][/b]")
        self._write_log(f"[b][cyan]📧 Generating Intro Email Drafts[/cyan][/b]")
        self._write_log(f"[dim]📄 Input: {Path(csv_path).name}[/dim]")
        if subject_template:
            ...
            if result.get("success"):
                ...
                if "content" in data and len(data["content"]) > 0:
                    ...
                    if response_data.get("ok"):
                        ...
                        self._write_log("[dim]Check your Outlook Drafts folder for the intro emails[/dim]")
                    else:
                        self._write_log(f"[yellow]⚠️  {response_data.get('message', 'Generation completed')}[/yellow]")
            else:
                error = result.get("error", "Unknown error")
