...
class RFQManagementScreen(Screen):
    ...
    async def on_mount(self):
        ...
        self._write_log(
            "[cyan]Ready. Select an action above or press 'r' for full workflow.[/cyan]"
        )
        self._write_log(
            "[dim]🤖 AI-powered analysis uses Claude/ChatGPT/Gemini for GO/NO-GO recommendations.[/dim]"
        )
        self._write_log("")
    ...
    async def action_download_attachments(self):
        ...
        try:
            ...
            if isinstance(res, dict):
                ...
                self._write_log(
                    f"[green]✅ Downloaded {len(att)} attachment(s) into {res.get('email_id','')}[/green]"
                )
            else:
    ...
    async def action_gen_internal_email(self):
        ...
        try:
            ...
            result = await asyncio.to_thread(
                self._call_tool, "create_rfq_drafts", {"rfq_id": int(rfq_id)}
            )

            if result and result.get("success"):
                self._write_log("[green]✅ Internal email draft created in Outlook[/green]")
    ...
    async def action_gen_oem_email(self):
        ...
        try:
            ...
            result = await asyncio.to_thread(
                self._call_tool, "rfq_draft_oem_registration", {"rfq_id": int(rfq_id)}
            )

            if result and result.get("success"):
                self._write_log("[green]✅ OEM registration email draft created in Outlook[/green]")
    ...
    async def _run_full_workflow(self):
        ...
        self._write_log(f"\n[b][cyan]{'═' * 80}[/cyan][/b]")
        self._write_log("[b][cyan]🚀 FULL RFQ WORKFLOW[/cyan][/b]")
        self._write_log(f"[dim]🕐 {datetime.now().strftime('%H:%M:%S')}[/dim]")
        ...
        if all_success:
            self._write_log("[b][green]✅ Full RFQ Workflow Completed Successfully![/green][/b]")
        else:
            self._write_log("[b][yellow]⚠️  Workflow Completed with Errors[/yellow][/b]")
